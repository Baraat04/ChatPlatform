...
- "Нужен ответ" ТОЛЬКО если ПОСЛЕДНЕЕ сообщение от КЛИЕНТА и требует реакции.
- "Лид" — диалог только начат и не завершён.
- "Отказ" — клиент явно отказался.
- "Молчит" — последнее сообщение от бота, клиент не ответил долго.
